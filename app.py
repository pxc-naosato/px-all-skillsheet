...
def parse_resume_with_ai(text_content: str):
    ...
    prompt = dedent("""
        ...
        【抽出ルール】
        - 氏名や名前は"name"に必ず抽出すること
        - フリガナは見つからなければ抽出しなくて良い。
        - フリガナは名前や氏名と同じ文字列ではないこと。
        - 日付は "YYYY-MM-DD" 形式。不明な場合は null または ""。
        - 性別は "男性", "女性", "その他", "未選択" のいずれか。
        - 資格は資格名だけ抽出し、改行区切りの1つの文字列にする。
        - 稼働可能日が明記されていない場合は今日の日付。
        - 案件リストは記載順に抽出。
        - 役割は文脈から["PM", "PL", "SPL", "SE", "PG"]のどれか判断して"role"に振り分ける。
        - 技術要素（OS, 言語, DBなど）は文脈から判断して振り分ける。不明なら "lang_tool" にまとめる。
        - 技術要素（OS, 言語, DBなど）で「REHL6,7,8」のようないった記載があった場合、「RHEL6」「RHEL7」といった形で別々に振り分けること
        - "scale"には人数のみ抽出すること。
