...
def parse_resume_with_ai(text_content: str):
    ...
    model = genai.GenerativeModel("gemini-3-flash-preview") # JSONモードが得意なモデル推奨
...
def parse_resume_with_ai_multimodal(content_input):
    ...
    base_prompt = dedent("""
        ...
        - Markdownのコードブロック（```json など）は絶対に含めないでください。
        - 文字列内の改行は必ずエスケープ文字（\\n）を使用し、制御文字を含めないでください。
